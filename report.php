...
switch ($reportType) {
	case 'daily-sales':
		...
		echo '<section><div class="report_data">' .
			 ...
			 '<input type="text" name="date" id="date" class="datepicker_no_future_date" size="30" maxlength="30" required="required" value="' .
			 		 $reportDateStr . '" />' .
			 '<input type="submit" name="submit_form" value="Go" />' .
			 '</form></div></section>';
		
		echo '<div id="tabs"><ul>' .
			 '<li><a href="#tab_content" onclick="report.loadReport(\'' . $reportType . '\',\'summary\',\'' .
			 		 $reportDateParam . '\')">Sales Summary</a></li>' .
			 '<li><a href="#tab_content" onclick="report.loadReport(\'' . $reportType . '\',\'edic-yokohama\',\'' .
			 		 $reportDateParam . '\')">EDIC Yokohama</a></li>' .
			 '<li><a href="#tab_content" onclick="report.loadReport(\'' . $reportType . '\',\'edic-others\',\'' .
			 		 $reportDateParam . '\')">EDIC Others</a></li>' .
			 '<li><a href="#tab_content" onclick="report.loadReport(\'' . $reportType . '\',\'mdj-yokohama\',\'' .
			 		 $reportDateParam . '\')">MDJ Yokohama</a></li>' .
			 '<li><a href="#tab_content" onclick="report.loadReport(\'' . $reportType . '\',\'mdj-others\',\'' .
			 		 $reportDateParam . '\')">MDJ Others</a></li>' .
			 '</ul>' .
		...
		<script type="text/javascript">
		<!--
			report.loadReport('<?=$reportType?>', '<?=$reportCategory?>', '<?=$reportDateParam?>');
			$('#date').focus(function () {
	...
	case 'periodic-sales':
		...
		echo '<section><div class="report_data"><form name="report_date_form" method="get" action="' . $formTarget . '" autocomplete="off">' .
			 '<input type="hidden" name="type" id="report_type" value="' . $reportType . '" />' .
			 '<input type="hidden" name="category" id="report_category" value="' . $reportCategory . '" />' .
		...
		echo '<div id="tabs"><ul>' .
			 '<li><a href="#tab_content" onclick="report.loadReport(\'' . $reportType . '\',\'summary\',\'' .
			 		 $startDateParam . '\',\'' . $endDateParam . '\')">Sales Summary</a></li>' .
			 '<li><a href="#tab_content" onclick="report.loadReport(\'' . $reportType . '\',\'edic-yokohama\',\'' .
			 		 $startDateParam . '\',\'' . $endDateParam . '\')">EDIC Yokohama</a></li>' .
			 '<li><a href="#tab_content" onclick="report.loadReport(\'' . $reportType . '\',\'edic-others\',\'' .
			 		 $startDateParam . '\',\'' . $endDateParam . '\')">EDIC Others</a></li>' .
			 '<li><a href="#tab_content" onclick="report.loadReport(\'' . $reportType . '\',\'mdj-yokohama\',\'' .
			 		 $startDateParam . '\',\'' . $endDateParam . '\')">MDJ Yokohama</a></li>' .
			 '<li><a href="#tab_content" onclick="report.loadReport(\'' . $reportType . '\',\'mdj-others\',\'' .
			 		 $startDateParam . '\',\'' . $endDateParam . '\')">MDJ Others</a></li>' .
			 '</ul><div id="tab_content"></div></div>';
		
		// load default report
		?>
		<script type="text/javascript">
		<!--
			report.loadReport('<?=$reportType?>', '<?=$reportCategory?>', '<?=$startDateParam?>', '<?=$endDateParam?>');
	...
	case 'projected-collections':
		...
		echo '<section><div class="report_data"><form name="report_date_form" method="get" action="' . $formTarget . '" autocomplete="off">' .
			 ...
			 '<input type="hidden" name="category" id="report_category" value="' . $reportCategory . '" />' .
			 '<label for="startdate">Start Date:</label>' .
			 '<input type="text" name="startdate" id="startdate" class="datepicker_no_past_date" size="30" 
			 		 maxlength="30" required="required" value="' . $startDateStr . '" />' .
			 '<label for="enddate">End Date:</label>' .
			 '<input type="text" name="enddate" id="enddate" class="datepicker_no_past_date" size="30" 
			 		 maxlength="30" required="required" value="' . $endDateStr . '" />' .
			 '<input type="submit" name="submit_form" value="Go" />' .
			 '</form></div></section>';
		
		echo '<div id="tabs"><ul>' .
			 '<li><a href="#tab_content" onclick="report.loadReport(\'' . $reportType . '\',\'incoming\',\'' .
			 		 $startDateParam . '\',\'' . $endDateParam . '\')">Incoming Cash</a></li>' .
			 '<li><a href="#tab_content" onclick="report.loadReport(\'' . $reportType . '\',\'outgoing\',\'' .
			 		 $startDateParam . '\',\'' . $endDateParam . '\')">Outgoing Cash</a></li>' .
			 '</ul><div id="tab_content"></div></div>';
	...
	case 'inventory':
		...
		echo '<br /><div id="tabs"><ul>' .
			 '<li><a href="#tab_content" onclick="report.loadReport(\'' . $reportType . '\',\'remaining\',\'' .
			 		 $startDateParam . '\',\'' . $endDateParam . '\'); report.loadFormEvents(); loadDatePicker()">Remaining Stocks</a></li>' .
			 '<li><a href="#tab_content" onclick="report.loadReport(\'' . $reportType . '\',\'to-buy-items\',\'' .
			 		 $startDateParam . '\',\'' . $endDateParam . '\'); report.loadFormEvents(); loadDatePicker()">To Buy</a></li>' .
			 '<li><a href="#tab_content" onclick="report.loadReport(\'' . $reportType . '\',\'sold-items\',\'' .
			 		 $startDateParam . '\',\'' . $endDateParam . '\'); report.loadFormEvents(); loadDatePicker()">Sold Items</a></li>' .
			 '<li><a href="#tab_content" onclick="report.loadReport(\'' . $reportType . '\',\'purchased-items\',\'' .
			 		 $startDateParam . '\',\'' . $endDateParam . '\'); report.loadFormEvents(); loadDatePicker()">Purchased Items</a></li>' . 
			 '</ul><div id="tab_content"></div></div>';
	...
	case 'rev-exp':
		...
		echo '<section><div class="report_data"><form name="report_date_form" method="get" action="' . $formTarget . '" autocomplete="off">' .
			 ...
			 '<input type="text" name="startdate" id="startdate" class="datepicker_no_future_date" size="30" 
			 		 maxlength="30" required="required" value="' . $startDateStr . '" />' .
			 '<label for="enddate">End Date:</label>' .
			 '<input type="text" name="enddate" id="enddate" class="datepicker_no_future_date" size="30" maxlength="30" 
			 		 required="required" value="' . $endDateStr . '" />' .
			 '<label for="viewtype">View by</label><select name="viewtype" id="viewtype">' .
			 '<option value="day">Day</option>' .
			 '<option value="week">Week</option>' .
			 '<option value="month">Month</option>' .
			 '<option value="quarter">Quarter</option>' .
			 '<option value="year">Year</option>' .
			 '</select><input type="submit" name="submit_form" value="Go" />' .
			 '</form></div></section>';
				
		echo '<div id="tabs"><ul>' .
			 '<li><a href="#tab_content" onclick="report.loadReport(\'' . $reportType . '\',\'orders-trend\',\'' .
			 		 $startDateParam . '\',\'' . $endDateParam . '\',\'' . $viewType . '\'); reloadChart(0)">Orders</a></li>' .
			 '<li><a href="#tab_content" onclick="report.loadReport(\'' . $reportType . '\',\'revenues-trend\',\'' .
			 		 $startDateParam . '\',\'' . $endDateParam . '\',\'' . $viewType . '\'); reloadChart(1)">Revenues</a></li>' .
			 '<li><a href="#tab_content" onclick="report.loadReport(\'' . $reportType . '\',\'purchases-trend\',\'' .
			 		 $startDateParam . '\',\'' . $endDateParam . '\',\'' . $viewType . '\'); reloadChart(2)">Purchases</a></li>' .
			 '<li><a href="#tab_content" onclick="report.loadReport(\'' . $reportType . '\',\'expenses-trend\',\'' .
			 		 $startDateParam . '\',\'' . $endDateParam . '\',\'' . $viewType . '\'); reloadChart(3)">Expenses</a></li>' .
			 '</ul><div id="tab_content"></div></div>';
		...
		<script type="text/javascript">
		<!--
			report.loadReport('<?=$reportType?>', '<?=$reportCategory?>', '<?=$startDateParam?>', '<?=$endDateParam?>', '<?=$viewType?>');
			report.setViewType('<?=$viewType?>');
			...
			reloadChart(<?=$selectedTab?>);		// initial call on page load
		// -->
		</script>
		<?php
		break;
	...
	case 'profit-calc':
		...
		?>
		<script type="text/javascript"><!--
			report.loadReport('profit-calc', null, null, null, null, 'report_content');
			report.setViewType('day'); 	// no bearing
			report.loadFormEvents();
