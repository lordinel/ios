...
END;
		...
		echo "<form name=\"" . ( $id == null ? "add" : "edit" ) . "_order\" method=\"post\" action=\"view_order_details.php\" autocomplete=\"off\" onsubmit=\"return order.validateInputForm(". ( $isItemEditable? 'true' : 'false' ) . ")\" onreset=\"return confirmReset('order.resetInputForm')\">\n";
...
END;
		...
		if ( $isItemEditable ) {
			echo <<<END
				order.toggleDeliveryPickupDateLabel();
...
END;
	...
	protected static function showInputFieldSet( $id = null, array $order = null, $isItemEditable = true )
	{
		...
		// display fieldset
		echo '<fieldset><legend>Order Info</legend>';
		echo '<section>';

		if ( $id != null )
		{
			echo '<div>';
			echo '<label for="order_number">Order No:</label>';
          	echo '<output name="order_number">' . $order['id'] . '</output>';
         	echo '</div>';
		}
...
?>			<div>
			...
			<div>
				...
				<select name="invoice_type" id="invoice_type" class="form_input_select">
					<option value="SI"<?php echo ( $id != null ? ( $order['sales_invoice'] != null ? " selected=\"selected\"" : "" ) : "" ) ?>>Sales Invoice (SI)</option>
					<option value="DR"<?php echo ( $id != null ? ( $order['delivery_receipt'] != null ? " selected=\"selected\"" : "" ) : "" ) ?>>Delivery Receipt (DR)</option>
				</select>
				<input type="hidden" name="invoice_type_orig" id="invoice_type_orig" value="<?php echo ( $id != null ) ? ( $order['sales_invoice'] != null ?  "SI" : "DR" ) : "null" ?>" />
			</div>
				
			<div>
				<label for="tracking_number"></label>
				<input type="text" name="tracking_number" id="tracking_number" required="required"<?php echo ( $id != null ) ? " value=\"" . ( $order['sales_invoice'] != null ?  Filter::reinput( $order['sales_invoice'] ) : Filter::reinput( $order['delivery_receipt'] ) ) . "\"" : "" ?> />
				<input type="hidden" name="tracking_number_orig" id="tracking_number_orig" value="<?php echo ( $id != null ) ? ( $order['sales_invoice'] != null ?  $order['sales_invoice'] : $order['delivery_receipt'] ) : "null" ?>" />
			</div>
		</section>
...
<?php
				...
				{
?>               <select name="transaction_type" id="transaction_type" class="form_input_select">
                    <option value="delivery"<?php echo ( $id != null ? ( $order['transaction_type'] == "delivery" ? " selected=\"selected\"" : "" ) : "" ) ?>>Delivery</option>
                    <option value="pick-up"<?php echo ( $id != null ? ( $order['transaction_type'] == "pick-up" ? " selected=\"selected\"" : "" ) : "" ) ?>>Pick-up</option>
                </select>
<?php			}
				else
				{
?>				<output><?php echo ucfirst( $order['transaction_type'] ) ?></output>
				<input type="hidden" name="transaction_type" id="transaction_type" value="<?php echo $order['transaction_type'] ?>" />
<?php			}
?>          </div>
...
<?php
				if ( $order['delivery_pickup_actual_date'] == null )
				{
?>				<input type="text" name="delivery_pickup_date" id="delivery_pickup_date" class="datepicker" size="30" maxlength="30" required="required"<?php echo ( $id != null ) ? " value=\"" . dateFormatOutput( Filter::reinput( $order['delivery_pickup_target_date'] ), "F j, Y, D" ) . "\"" : "" ?> />
				<span class="span_label">Time:</span>
				<select name="delivery_pickup_time" id="delivery_pickup_time">
					<option value="0:00:00"<?php echo ( $id != null ) ? ( dateFormatOutput( $order['delivery_pickup_target_date'], "G:i:s" ) == "0:00:00" ? " selected=\"selected\"" : "" ) : "" ?>>----------&nbsp;&nbsp;</option>
<?php				
...
<?php			}
				else
				{
					...
					echo '<input type="hidden" name="delivery_pickup_date" id="delivery_pickup_date" value="' . dateFormatOutput( Filter::reinput( $order['delivery_pickup_target_date'] ), "F j, Y, D" ) . '" />';
					echo '<input type="hidden" name="delivery_pickup_time" id="delivery_pickup_time" value="' . dateFormatOutput( $order['delivery_pickup_target_date'], "G:i:s" ) . '" />';
					echo '<input type="hidden" name="delivery_pickup_actual_time" id="delivery_pickup_actual_time" value="' . $order['delivery_pickup_actual_date'] . '" />';
				}
?>				
			</div>
		</section>
...
END;
					if ((int)$item[$i - 1]['undelivered_quantity'] < (int)$item[$i - 1]['quantity']) {
						echo <<<END
							$('#item_brand_'+$i).attr( 'disabled', 'disabled' );
...
END;
			...
			?><tr>
                    ...
                    <tr>
						<td colspan="6"><label for="total_amount">Total: <?php echo CURRENCY ?></label></td>
                        <td><input type="text" name="total_sidr_amount" id="total_sidr_amount" class="number output_field" value="<?php echo ($id != null) ?
				numberFormat(Filter::reinput($totalSIDRamount), 'float', 3, '.', '', true) : "0.000" ?>" disabled="disabled" /></td>
                        <td></td>
                        <td><input type="text" name="total_net_amount" id="total_net_amount" class="number output_field" value="<?php echo ($id != null) ?
				numberFormat(Filter::reinput($totalNetAmount), 'float', 3, '.', '', true) : "0.000" ?>" disabled="disabled" /></td>
					</tr>
					...
					<tr class="tax_fields">
						<td colspan="8"><label for="total_sales">Total Sales: <?php echo CURRENCY ?></label></td>
						<td><input type="text" name="total_sales" id="total_sales" class="number order_item_total output_field" value="<?php echo ($id !=
																																				   null) ?
				numberFormat(Filter::reinput($order['total_sales_amount']), 'float', 3, '.', '', true) : "0.000" ?>" disabled="disabled" /></td>
                        <td></td>
					</tr>
                    <tr class="tax_fields">
						<td colspan="8"><label for="value_added_tax">+ Value-Added Tax: <?php echo CURRENCY ?></label></td>
						<td><input type="text" name="value_added_tax" id="value_added_tax" class="number order_item_total output_field" value="<?php echo ($id !=
																																						   null) ?
				numberFormat(Filter::reinput($order['value_added_tax']), 'float', 3, '.', '', true) : "0.000" ?>" disabled="disabled" /></td>
                    </tr>
					<tr class="tax_fields">
						<td colspan="9"><br /></td>
                    </tr>
					...
					<tr>
						<td colspan="8"><label for="sidr_amount" class="important_label">SI/DR Amount: <?php echo CURRENCY ?></label></td>
						<td><input type="text" name="sidr_amount" id="sidr_amount" class="number order_item_total output_field" value="<?php echo ($id !=
																																				   null) ?
				numberFormat($totalSIDRamount, 'float', 3, '.', '', true) : "0.000" ?>" disabled="disabled" /></td>
					</tr>
					...
					<tr class="tax_fields">
						<td colspan="8"><label for="withholding_tax">- Withholding Tax: <?php echo CURRENCY ?></label></td>
						<td><input type="text" name="withholding_tax" id="withholding_tax" class="number order_item_total" value="<?php echo ($id != null) ?
				numberFormat(Filter::reinput($order['withholding_tax']), 'float', 3, '.', '', true) : "0.000" ?>" disabled="disabled" /></td>
					</tr>
					<tr class="tax_fields">
						<td colspan="9"><br /></td>
					</tr>
                    <tr>
						<td colspan="8"><label for="net_amount" class="important_label">OFC Net Amount: <?php echo CURRENCY ?></label></td>
						<td><input type="text" name="net_amount" id="net_amount" class="number order_item_total output_field" value="<?php echo ($id != null) ?
				numberFormat(($totalNetAmount - $order['withholding_tax']), 'float', 3, '.', '', true) : "0.000" ?>" disabled="disabled" /></td>
					</tr>
				</tbody>
			</table>
		</section>
		...
		<!--
			order.setMaxItems( <?php echo self::MAX_ORDER_ITEMS ?> );
			order.setInitialVisibleItems( <?php echo $visibleItems ?> );
			order.payment.setVAT( '<?php echo VAT_PERCENT ?>' );
<?php
			if ($id != null) {
				echo <<<END
					$('#total_sales').attr( 'defaultValue', $('#total_sales').val() );
...
<?php
			...
			echo "<section><div id=\"horizontal_message\">Items are already delivered or payment is already made.<br />" .
			     "Return delivered items and cancel all payments to edit
 Order Items and Payment details.</div></section>"; 
		}
	}
	...
	public function save( $customerID )
	{
		...
		if ($_POST['order_query_mode'] == "new")			// new transaction
		{
			// save new order
			$sqlQuery = "INSERT INTO `order` VALUES (" .
						...
						($this->businessUnit == null ?
							"NULL" : "'" . $this->businessUnit . "'") . 
						"," .				// business unit
						($this->branch == null ?
							"NULL" : "'" . $this->branch . "'") . ",'" .				    // branch
			...
			//$orderNumber = str_pad( $this->id, 10, '0', STR_PAD_LEFT );
			$orderNumber = $this->id;
		...
		else				// existing transaction, update records
		{
			...
			if ( $isItemEditable ) {
				$sqlQuery = "UPDATE `order` SET ".
							...
							"business_unit=". ($this->businessUnit == null ?
								"NULL" : "'".$this->businessUnit."'").",".                    			// business unit
...
<?php
	...
	public static function showList()
	{
		...
		$sqlQuery = "SELECT `order`.*, " .
					...
					"LEFT JOIN v_rebate_pdc_payable ON `order`.id = v_rebate_pdc_payable.order_id " .
					$condition . " " .
					"GROUP BY `order`.id ORDER BY " . $sortColumn . " " . $sortMethod .
		...
		if ( self::$database->getResultCount( $resultSet ) > 0 ) {
			...
			self::showPagination( $page, $itemsPerPage, $resultCount, 'order_list_section', 'Order::showList', $criteria, $sortColumn, $sortMethod, $filterName, $filterValue );
			echo '</div>';*/
			...
			while ( $order = self::$database->getResultRow( $resultSet ) ) {
				...
				if ( $filterName != "customer_id" ) {
					echo '<td>' .
						 ...
						 '<a href="view_customer_details.php?id=' . $order['customer_id'] . '" title="' . capitalizeWords( Filter::output( $order['customer'] ) ) . '">' .
				...
				if ( $order['canceled_date'] != NULL ) {
					...
					/*if ( $order['rebate_payable'] > 0 ) {
						echo '<img src="images/rebate.png" class="status_icon" title="With pending rebate: ' . numberFormat( $order['rebate_payable'], "currency" ) . '" />';
					} elseif ( $order['pdc_rebate_payable'] > 0 ) {
						echo '<img src="images/rebate.png" class="status_icon" title="With rebate to clear: ' . numberFormat( $order['pdc_rebate_payable'], "currency" ) . '" />';
					} elseif ( $order['cleared_date'] != NULL ) {
						echo '<img src="images/success.png" class="status_icon" title="Order is cleared" />';
					}*/
				}
				echo '</td>';
				...
				if ( $order['canceled_date'] != NULL ) {
					...
					if ( $order['pdc_rebate_payable'] > 0 ) {
						echo '<span class="bad">' . numberFormat( $order['rebate_payable'], "float" ) . '</span>' .
							 '<img src="images/rebate.png" class="status_icon" title="Rebate to clear: ' . numberFormat( $order['pdc_rebate_payable'], "currency", '3', '.', ',', true ) . '" />';
					} else {
		...
		if ( self::$database->getResultCount( $resultSet ) > 0 ) {
			while ( $order = self::$database->getResultRow( $resultSet ) ) {
				...
				if ( $order['delivery_pickup_actual_date'] != null ) {
					...
					$activeSheet->getComment( 'I' . $rowPtr )->getText()
						->createTextRun( 'Order is not yet '.( $order['transaction_type'] == 'delivery' ? 'delivered to customer' : 'picked-up by customer' ) )
						->getFont()->setSize( EXCEL_COMMENT_FONT_SIZE );
				}
	...
	// tasks for order details
	public function showDetailsTasks()
	{
		$sqlQuery = "SELECT id, transaction_type, balance, IF(canceled_date IS NOT NULL,'canceled',IF(cleared_date IS NOT NULL,'cleared',IF(delivery_pickup_actual_date IS NULL,'not-delivered','all-delivered'))) AS order_status, IF(balance <= 0,'fully-paid',IF(balance = amount_receivable,'no-payment','partially-paid')) AS payment_status FROM `order` WHERE `order`.id = " . $this->id;
		$resultSet = self::$database->query( $sqlQuery );
		$order = self::$database->getResultRow( $resultSet );
...
?>		<div id="tasks">
			<ul>
				<li id="task_edit_order" style="display: none"><a href="edit_order.php?id=<?php echo $order['id'] ?>">
				...
				<li id="task_cancel_order" style="display: none"><a href="javascript:void(0)" onclick="showDialog('Change Status','<?php
					...
					// Yes and No buttons
					$dialogMessage = $dialogMessage . "<div id=\"dialog_buttons\">";
					$dialogMessage = $dialogMessage . "<input type=\"button\" value=\"Yes\" onclick=\"cancelOrderCommit(\'" . $order['id'] . "\')\" />";
				...
				<li id="task_undo_cancel_order" style="display: none"><a href="javascript:void(0)" onclick="showDialog('Change Status','<?php
					...
					// Yes and No buttons
					$dialogMessage = $dialogMessage . "<div id=\"dialog_buttons\">";
					$dialogMessage = $dialogMessage . "<input type=\"button\" value=\"Yes\" onclick=\"undoCancelOrderCommit(\'" . $order['id'] . "\')\" />";
					$dialogMessage = $dialogMessage . "<input type=\"button\" value=\"No\" onclick=\"hideDialog()\" />";
					$dialogMessage = $dialogMessage . "</div>";
				...
				<li id="task_mark_as_cleared" style="display: none"><a href="javascript:void(0)" onclick="showDialog('Change Status','<?php
					...
					// Yes and No buttons
					$dialogMessage = $dialogMessage . "<div id=\"dialog_buttons\">";
					$dialogMessage = $dialogMessage . "<input type=\"button\" value=\"Yes\" onclick=\"markAsClearedCommit(\'" . $order['id'] . "\')\" />";
					$dialogMessage = $dialogMessage . "<input type=\"button\" value=\"No\" onclick=\"hideDialog()\" />";
					$dialogMessage = $dialogMessage . "</div>";
				...
				<li id="task_unclear_order" style="display: none"><a href="javascript:void(0)" onclick="showDialog('Change Status','<?php
					...
					// Yes and No buttons
					$dialogMessage = $dialogMessage . "<div id=\"dialog_buttons\">";
					$dialogMessage = $dialogMessage . "<input type=\"button\" value=\"Yes\" onclick=\"unclearOrderCommit(\'" . $order['id'] . "\')\" />";
					$dialogMessage = $dialogMessage . "<input type=\"button\" value=\"No\" onclick=\"hideDialog()\" />";
					$dialogMessage = $dialogMessage . "</div>";
				...
				<li id="task_export"><a href="javascript:void(0)" onclick="showDialog('Export to Excel','<?php
					...
					$dialogMessage = $dialogMessage . '<div id="dialog_buttons">' .
						'<input type="button" value="Yes" onclick="exportToExcelConfirm(' .
						'\\\'data=order_details&orderID=' . $this->id . '\\\')" />' .
						'<input type="button" value="No" onclick="hideDialog()" />' .
						'</div>';
		...
		<!--
			...
			orderStatus = '<?php echo $order['order_status'] ?>';
			paymentStatus = '<?php echo $order['payment_status'] ?>';
			transactionType = '<?php echo $order['transaction_type'] ?>';
			...
			$(document).ready( function () {
				...
				if (typeof hideTasks == "function") {
					// hide tasks if no permission to view the order
					hideTasks();
				}
			});
		// -->
...
<?php
	...
	public function view()
	{
		// get main order info
		$sqlQuery = "SELECT `order`.*, " .
					"IF(canceled_date IS NOT NULL,'canceled',IF(cleared_date IS NOT NULL,'cleared',IF(delivery_pickup_actual_date IS NULL,'pending',IF(balance = 0,'fully-paid',IF(balance = amount_receivable,'delivered','partially-paid'))))) AS status, " . 
...
?>	<fieldset><legend>Order Info</legend>
		<section class="main_record_label">
			<div><?php echo "Order No. " . $order['id'] ?></div>
		</section>
		...
		<section>
			<div>
				...
				?></span>
				<span class="record_data"><b><?php echo $trackingNumber ?></b></span>
			</div>
			<div>
				<span class="record_label">Customer:</span>
				<span class="record_data"><a href="view_customer_details.php?id=<?php echo $order['customer_id'] ?>"><?php echo capitalizeWords( Filter::output( $order['customer_name'] ) ) ?></a></span>
			</div>
			<div>
		...
		<section>
			<div>
				...
				<span class="record_data"><?php echo ucfirst( $order['transaction_type'] ) ?></span>
			</div>
			<div>
				<span class="record_label"><?php echo ucfirst( $order['transaction_type'] ) ?> Date:</span>
				<span class="record_data"><?php
					$hour = (int) dateFormatOutput( $order['delivery_pickup_target_date'], "G" );

					if ( $hour < WORKING_HOURS_START || $hour > WORKING_HOURS_END )		// hour is outside working hours, do not display time
...
<?php
			...
			$sqlQuery = $sqlQuery . " INNER JOIN inventory ON inventory.id = order_item.inventory_id";
			$sqlQuery = $sqlQuery . " INNER JOIN inventory_brand ON inventory_brand.id = inventory.brand_id";
			$sqlQuery = $sqlQuery . " WHERE order_item.order_id = " . $this->id;
                        ...
                        <td class="quantity_link"><?php
							if ( $order['status'] != "cleared" )
							{
                            	echo '<span class="item_delivery_link">' . 
									 '<a href="javascript:void(0)" onclick="showDialog(\'Mark Item as ' . ( $order['transaction_type'] == "delivery" ? "Delivered" : "Picked-up" ) . '\',\'Getting data...\',\'prompt\'), ajax(null,\'dialog_message\',\'innerHTML\',\'Order::showItemDeliveryDialog\',\'class=order&transactionID=' . $order['id'] . '&itemID=' . $item['id'] . '&index=' . $i . '\'), loadDatePicker()" >' .
                                	 '<span id="item_quantity_' . $i . '">' . numberFormat( $item['quantity'] - $item['undelivered_quantity'], "int" ) . '</span>' .
                                 	 '</a></span>';
							}
							else
								echo '<span id="item_quantity_' . $i . '">' . numberFormat( $item['quantity'] - $item['undelivered_quantity'], "int" ) . '</span>';
...
<?php
					if ( $order['status'] != "cleared" )
					{
						?><span class="item_delivery_link"><a href="javascript:void(0)" onclick="showDialog('Mark All Items as <?php echo ( $order['transaction_type'] == "delivery" ? "Delivered" : "Picked-up" ) ?>','Getting data...','prompt'), ajax(null,'dialog_message','innerHTML','Order::showAllItemsDeliveryDialog','class=order&transactionID=<?php echo $order['id'] ?>&maxIndex=<?php echo ( $i - 1 ) ?>'), loadDatePicker()" ><span id="item_quantity_total"><?php echo $totalDeliveredItems ?></span></a></span><?php
					}
					else
					{
...
?>              <tr>
					...
					<td class="number"><span><?php echo numberFormat( $order['total_sales_amount'], "float" ) ?></span></td>
				</tr>
                <tr>
					<td colspan="8" class="summary_label">+ Value-Added Tax: <?php echo CURRENCY ?></td>
					<td class="number"><span><?php echo numberFormat( $order['value_added_tax'], "float" ) ?></span></td>
				</tr>
                <tr>
					<td colspan="8" class="summary_label">- Withholding Tax: <?php echo CURRENCY ?></td>
					<td class="number"><span><?php echo numberFormat( $order['withholding_tax'], "float" ) ?></span></td>
				</tr>
<?php			}
...
?>				<tr>
				...
				<tr>
					...
					<td class="number"><span><?php echo numberFormat( $order['interest'], "float" ) ?></span></td>
				</tr>
<?php			}
?>
				<tr>
                ...
                <tr class="totals_top">
					...
					<td class="number"><span class="important_label"><?php echo numberFormat( $order['receipt_amount'], "float" ) ?></span></td>
				</tr>
                <tr class="totals_bottom">
					...
					<td class="number"><span class="important_label"><?php echo numberFormat( $order['amount_receivable'], "float" ) ?></span></td>
				</tr>
			</tbody>
		</table>
	</section>
...
<?php	}
		else				// order number is not existing, redirect to home page
		{
			?><fieldset><legend>Order Info</legend>
				<section><div><br />
				<span class="now">Error:</span> You do not have permission to view Order No. <?php
					echo $this->id;
	...
	public static function markAsCleared()
	{
		...
		if ( self::$database->query( $sqlQuery ) ) {
			...
			EventLog::addEntry( self::$database, 'info', 'order', 'update', 'cleared',
								...
								'<a href="view_customer_details.php?id=' . $order['customer_id'] . '">' . capitalizeWords( Filter::output( $order['name'] ) ) . '</a> ' .
								'was <span class="event_log_action good">cleared</span>' );
	...
	public static function undoClear()
	{
		...
		if ( self::$database->query( $sqlQuery ) ) {
			...
			EventLog::addEntry( self::$database, 'info', 'order', 'update', 'uncleared',
								...
								$_POST['orderID'] . '</a> (' . $invoiceNumber . '): </span>Order from ' .
								'<a href="view_customer_details.php?id=' . $order['customer_id'] . '">' . capitalizeWords( Filter::output( $order['name'] ) ) . '</a> ' .
								'was <span class="event_log_action">uncleared</span>' );
	...
	public static function cancel()
	{
		...
		{
			$sqlQuery = "UPDATE inventory SET reserved_stock = IF( reserved_stock >= " . $orderItem['quantity'] . ", reserved_stock - " . $orderItem['quantity'] . ", 0) " .
		...
		if ( self::$database->query( $sqlQuery ) ) {
			...
			EventLog::addEntry( self::$database, 'info', 'order', 'update', 'canceled',
								...
								$_POST['orderID'] . '</a> (' . $invoiceNumber . '): </span>Order from ' .
								'<a href="view_customer_details.php?id=' . $order['customer_id'] . '">' . capitalizeWords( Filter::output( $order['name'] ) ) . '</a> ' .
								'was <span class="event_log_action bad">canceled</span>' );
	...
	// undo cancel order, ajax function
	public static function undoCancel()
	{
		...
		EventLog::addEntry( self::$database, 'info', 'order', 'update', 'uncanceled',
							...
							$_POST['orderID'] . '</a> (' . $invoiceNumber . '): </span>Order from ' .
							'<a href="view_customer_details.php?id=' . $order['customer_id'] . '">' . capitalizeWords( Filter::output( $order['name'] ) ) . '</a> ' .
							'was <span class="event_log_action">uncanceled</span>' );
