...
class Report extends Layout
{
	...
	private static function showDailySalesReport( $category, $date )
	{
		...
		if ( $category == 'summary' ) {
			...
			echo <<<END
				<br />
				<table id="daily_sales_table" class="item_input_table report_table">
					<thead>
						<tr>
							<th id="summary_category">Catergory</th>
							<th id="summary_quantity">Total Quantity</th>
							<th id="summary_amount">Total Amount</th>
						</tr>
					</thead>
					<tbody>
...
END;
			...
			switch ( $category ) {
				case 'edic-yokohama': {
					$condition = "inventory_brand.id = " . $yokohamaID . " AND `order`.business_unit = 'EDIC'";
					break;
				}
				case 'edic-others': {
					$condition = "inventory_brand.id != " . $yokohamaID . " AND `order`.business_unit = 'EDIC'";
				...
				case 'mdj-yokohama': {
					$condition = "inventory_brand.id = " . $yokohamaID . " AND `order`.business_unit = 'MDJ'";
					break;
				}
				case 'mdj-others': {
					$condition = "inventory_brand.id != " . $yokohamaID . " AND `order`.business_unit = 'MDJ'";
...
END;
			...
			foreach ( $orderList as $order ) {
				...
				$orderCount = self::$database->getResultCount( $resultSet );
			...
			echo <<<END
						...
						<tr>
							<td colspan="3" class="summary_label">
								<span class="important_label">Totals:</span>
							</td>
							<td class="number">
								<span class="important_label">$totalQuantity</span>
							</td>
							<td colspan="4" class="summary_label">
								<span class="important_label">$currency</span>
							</td>
							<td class="number">
								<span class="important_label">$totalNetAmount</span>
							</td>
						</tr>
					</tbody>
				</table>
END;
	...
	private static function showPeriodicSalesReport( $category, $startDate, $endDate )
	{
		...
		echo '<div id="date_header">From ' .
			 '<span class="date">' . $startDateStr . '</span> to ' .
...
END;
			...
			$sqlQuery = "SELECT DATE_FORMAT(delivery_pickup_actual_date,'%M %e, %Y, %a') AS delivery_pickup_actual_date, `order`.id, " .
						...
						"INNER JOIN inventory ON order_item.inventory_id = inventory.id " .
						"INNER JOIN inventory_brand ON inventory.brand_id = inventory_brand.id " .
						"WHERE (`order`.delivery_pickup_actual_date BETWEEN '" . $startDate . " 00:00:00' " . "AND '" . $endDate . " 23:59:59') AND " . $condition . " " .
						"GROUP BY `order`.id " .
				...
				if ( $order['delivery_pickup_actual_date'] != $previousDate ) {     // same date division
					if ( $previousDate != null ) {
						...
						echo <<<END
								...
								<tr>
									<td colspan="3" class="summary_label">
										<span class="important_label">Subtotal:</span>
									</td>
									<td class="number">
										<span class="important_label">$subtotalQuantity</span>
									</td>
									<td colspan="4" class="summary_label">
										<span class="important_label">$currency</span>
									</td>
									<td class="number">
										<span class="important_label">$subtotalNetAmount</span>
									</td>
								</tr>
							</tbody>
						</table>
END;
					...
					echo '<br /><div class="report_date_header"><a href="report.php?type=daily-sales&category=' . $category . '&date=' . urldecode( $order['delivery_pickup_actual_date'] ) . '&src=periodic-sales" title="View Daily Sales">' . $order['delivery_pickup_actual_date'] . '</a></div>';
...
END;
			...
			echo <<<END
						...
						<tr style="border-top: 1px solid #000000">
							<td colspan="3" class="summary_label">
								<span class="important_label">Grand Total:</span>
							</td>
							<td class="number">
								<span class="important_label">$totalQuantity</span>
							</td>
				...
				if ( $previousDate != null ) {
					...
					echo <<<END
								...
								<tr>
									...
									<td class="number">
										<span class="important_label">$subtotalAmount</span>
									</td>
								</tr>
							</tbody>
						</table>
END;
				...
				if ( $category == 'incoming') {
					echo <<<END
						<br />
						<table id="projected_collections_table" class="item_input_table report_table">
							<thead>
								<tr>
									<th id="order_number">Order No.</th>
									<th id="tracking_number">Invoice No.</th>
...
END;
			...
			if ( $category == 'incoming') {
				...
				echo '<td></td>'; // filler cell
				echo '<td class="number"><span>' . numberFormat( $order['amount_to_collect'], 'float' ) . '</span></td>';
				...
				echo '<td>' . $order['tracking_number'] . '</td>';
				echo '<td><a href="view_supplier_details.php?id=' . $order['supplier_id'] . '">' .
                     capitalizeWords(Filter::output( $order['supplier_name'])).'</a></td>';
				echo '<td></td>'; // filler cell
				echo '<td class="number"><span>' . numberFormat( $order['amount_to_pay'], 'float' ) . '</span></td>';
		...
		echo <<<END
				...
				<tr style="border-top: 1px solid #000000">
					...
					<td class="number">
						<span class="important_label">$totalAmount</span>
					</td>
				</tr>
			</tbody>
		</table>
...
END;
		...
		switch ( $category ) {
			case 'remaining': {
				...
				echo '<div id="date_header">As of Today, ' .
				...
				echo <<<END
					<br />
					<table id="inventory_remaining_table" class="item_input_table report_table">
						<thead>
							<tr>
								<th id="brand">Brand</th>
...
END;
				...
				echo <<<END
							...
							<tr>
								...
								<td class="number">
									<span class="important_label">$totalStockAmount</span>
								</td>
								<td class="number">
									<span class="important_label">$totalExpectedRevenues</span>
								</td>
								<td class="number">
									<span class="important_label">$totalExpectedIncome</span>
								</td>
								<td class="number">
									<span class="important_label">$totalReservation</span>
								</td>
							</tr>
						</tbody>
					</table>			
END;
			...
			case 'to-buy-items': {
				...
				echo <<<END
					<br />
					<table id="inventory_to_buy_table" class="item_input_table report_table">
						<thead>
							<tr>
								<th id="brand">Brand</th>
								<th id="model">Model</th>
...
END;
				...
				while( $report = self::$database->getResultRow( $resultSet ) ) {
					echo '<tr class="item_row">';
					echo '<td>' . capitalizeWords(Filter::output($report['name'])) . '</td>';
					echo '<td><a href="list_inventory_orders.php?inv-id=' . $report['id'] . '" target="top">' .
					...
					echo '<td class="number"><span>' . numberFormat( $report['reserved_stock'], 'int', 0 ) . '</span></td>';
					echo '<td class="number"><span class="bad">' . numberFormat( $report['missing_stock'], 'int', 0 ) . '</span></td>';
					$totalQuantity = $totalQuantity + $report['missing_stock'];
					echo '<td class="number"><span>' . numberFormat( $report['amount'], 'float' ) . '</span></td>';
...
END;
			...
			case 'sold-items': {
				...
				echo <<<END
					<table id="inventory_sold_table" class="item_input_table report_table">
						<thead>
							<tr>
								<th id="brand">Brand</th>
								<th id="model">Model</th>
...
END;
				...
				while( $report = self::$database->getResultRow( $resultSet ) ) {
					echo '<tr class="item_row">';
					echo '<td>' . capitalizeWords(Filter::output($report['name'])) . '</td>';
					echo '<td>' . capitalizeWords(Filter::output($report['model'])) . '</td>';
					echo '<td class="number"><span>' . numberFormat( $report['delivered_quantity'], 'int', 0 ) . '</span></td>';
					$totalQuantity = $totalQuantity + $report['delivered_quantity'];
					echo '<td class="number"><span>' . numberFormat( $report['sold_amount'], 'float' ) . '</span></td>';
				...
				echo <<<END
							...
							<tr>
								<td colspan="2" class="summary_label">
									<span class="important_label">Totals:</span>
								</td>
								<td class="number">
									<span class="important_label">$totalQuantity</span>
...
END;
			...
			case 'purchased-items': {
				$sqlQuery = "SELECT inventory_brand.name, " .
							...
							"INNER JOIN inventory_brand ON inventory.brand_id = inventory_brand.id " .
							"WHERE (purchase_date BETWEEN '" . $startDate->format( "Y-m-d" ) . " 00:00:00' AND '" . $endDate->format( "Y-m-d" ) . " 23:59:59') AND " .
							"canceled_date IS NULL " .
...
END;
				...
				while( $report = self::$database->getResultRow( $resultSet ) ) {
					...
					echo '<td>' . capitalizeWords(Filter::output($report['name'])) . '</td>';
					echo '<td>' . capitalizeWords(Filter::output($report['model'])) . '</td>';
					echo '<td class="number"><span>' . numberFormat( $report['purchased_quantity'], 'int', 0 ) . '</span></td>';
					$totalQuantity = $totalQuantity + $report['purchased_quantity'];
					echo '<td class="number"><span>' . numberFormat( $report['purchased_amount'], 'float' ) . '</span></td>';
...
END;
		...
		switch ( $viewType ) {
			...
			case 'quarter': {
				...
				switch ( (int) $startDate->format( "m" ) ) {
					case  1: case  2: case  3: $startQuarter = 1; break;
					case  4: case  5: case  6: $startQuarter = 2; break;
					case  7: case  8: case  9: $startQuarter = 3; break;
					case 10: case 11: case 12: $startQuarter = 4; break;
				}

				switch ( (int) $endDate->format( "m" ) ) {
					case  1: case  2: case  3: $endQuarter = 1; break;
					case  4: case  5: case  6: $endQuarter = 2; break;
					case  7: case  8: case  9: $endQuarter = 3; break;
					case 10: case 11: case 12: $endQuarter = 4; break;
				}
		...
		switch ( $category ) {
			...
			case 'revenues-trend': {
				...
				echo '<div>' .
					 ...
					 numberFormat( ( ( $clearedPayments + $amountReceivable + $pdcReceivable ) -
					 				 ( $clearedRebates + $rebatePayable + $pdcRebatePayable ) ),
					 			   'currency' ) .
					 '</b></span>' .
					 '</div>';
	...
	private static function iterateChartData( $sqlQuery,
											  $viewType,
											  DateTime $startDate,
											  DateTime $endDate,
...
END;
		...
		for ( $i = 1; $i <= 5; $i++ ) {
			echo '<tr>';
			echo '<td>' . $i . '.</td>';
			echo '<td><input type="text" name="bank_name_' . $i . '" id="bank_name_' . $i . '" /></td>';
			echo '<td><input type="text" name="bank_amount_' . $i . '" id="bank_amount_' . $i . '" value="0.000" class="number bank_amount" /></td>';
			echo '</tr>';
		}
		...
		$currentDate = new DateTime();
		$currentDate = urlencode( $currentDate->format( "F j, Y, D" ) );
		
		$startDate = new DateTime();
		$startDate->sub( new DateInterval('P10Y') );
		$startDate = urlencode( $startDate->format( "F j, Y, D" ) );
		...
		echo <<<END
						...
						<tr>
							<td>+</td>
							<td><label for="amount_receivable"><a href="list_orders.php?criteria=all-orders" target="_blank">Amount Receivable</a>: $currency</label></td>
							<td><input type="text" name="amount_receivable" id="amount_receivable" class="number output_field" value="$amountReceivable" disabled="disabled" /></td>
						</tr>
						<tr>
							<td>+</td>
							<td><label for="pdc_receivable"><a href="list_orders.php?criteria=all-orders" target="_blank">PDC Receivable</a>: $currency</label></td>
							<td><input type="text" name="pdc_receivable" id="pdc_receivable" class="number output_field" value="$pdcReceivable" disabled="disabled" /></td>
						</tr>
						<tr>
							<td>+</td>
							<td><label for="stock_amount"><a href="report.php?type=inventory&category=remaining" target="_blank">Inventory Amount</a>: $currency</label></td>
							<td><input type="text" name="inventory_amount" id="inventory_amount" class="number output_field" value="$inventoryAmount" disabled="disabled" /></td>
						</tr>
						<tr>
							<td colspan="3"></td>
						</tr>
						<tr>
							<td>-</td>
							<td><label for="amount_payable"><a href="list_purchases.php?criteria=all-purchases" target="_blank">Amount Payable</a>: $currency</label></td>
							<td><input type="text" name="amount_payable" id="amount_payable" class="number output_field" value="$amountPayable" disabled="disabled" /></td>
						</tr>
						<tr>
							<td>-</td>
							<td><label for="pdc_payable"><a href="list_purchases.php?criteria=all-purchases" target="_blank">PDC Payable</a>: $currency</label></td>
							<td><input type="text" name="pdc_payable" id="pdc_payable" class="number output_field" value="$pdcPayable" disabled="disabled" /></td>
						</tr>
						<tr>
							<td>-</td>
							<td><label for="rebate_payable"><a href="list_orders.php?criteria=all-orders" target="_blank">Rebate Payable</a>: $currency</label></td>
							<td><input type="text" name="rebate_payable" id="rebate_payable" class="number output_field" value="$rebatePayable" disabled="disabled" /></td>
						</tr>
						<tr>
							<td>-</td>
							<td><label for="pdc_rebate"><a href="list_orders.php?criteria=all-orders" target="_blank">PDC Rebate Payable</a>: $currency</label></td>
							<td><input type="text" name="pdc_rebate" id="pdc_rebate" class="number output_field" value="$pdcRebate" disabled="disabled" /></td>
						</tr>
						<tr>
							<td>-</td>
