...
class Agent extends Person
{
	...
	{
		echo "<form name=\"" . ( $id == null ? "add" : "edit" ) . "_agent\" method=\"post\" action=\"view_agent_details.php\" autocomplete=\"off\" onreset=\"return confirmReset('resetAgentForm')\">\n";
...
?>
		<section>
			<div>
				...
				<input type="text" name="branch" id="branch" class="form_input_text" maxlength="100"<?php echo ( $agentInfo != null ) ? " value=\"" . capitalizeWords( Filter::reinput( $agentInfo['branch'] ) ) . "\"" : "" ?> />
			</div>
			<div>
				<label for="department">Department:</label>
				<input type="text" name="department" id="department" class="form_input_text" maxlength="100"<?php echo ( $agentInfo != null ) ? " value=\"" . capitalizeWords( Filter::reinput( $agentInfo['department'] ) ) . "\"" : "" ?> />
			</div>
			<div>
				<label for="position">Position:</label>
				<input type="text" name="position" id="position" class="form_input_text" maxlength="100"<?php echo ( $agentInfo != null ) ? " value=\"" . capitalizeWords( Filter::reinput( $agentInfo['position'] ) ) . "\"" : "" ?> />
			</div>
		</section>
<?php
		echo "</fieldset>\n";
	...
	{
		if ( $_POST['agent_query_mode'] == "new" )		// new customer
		{
			...
			EventLog::addEntry( self::$database, 'info', 'agent', 'insert', 'new',
								'<span class="event_log_main_record_inline"><a href="view_agent_details.php?id=' . $this->id . '">' .
								capitalizeWords( htmlentities( $_POST['agent_name'] ) ) .
								'</a></span> was <span class="event_log_action">added</span> to <a href="list_agents.php">Agents</a>' );
		}
		elseif ( $_POST['agent_query_mode'] == "edit" )		// existing customer, update records
		{
			...
			EventLog::addEntry( self::$database, 'info', 'agent', 'update', 'modified',
								'Agent <span class="event_log_main_record_inline"><a href="view_agent_details.php?id=' . $this->id . '">' .
								capitalizeWords( htmlentities( $_POST['agent_name'] ) ) .
...
<?php
	...
	public function view()
	{
		...
		if ( self::$database->getResultCount( $resultSet ) > 0 )		// agent ID is found in database
		{
...
?>
	...
	<script type="text/javascript">
		ajax( null, 'order_list_section', 'innerHTML', 'Order::showList', 'criteria=all-orders&filterName=agent_id&filterValue=<?php echo $this->id ?>' );
	</script>
<?php
			}
...
?>
	...
	<script type="text/javascript">
		ajax( null, 'purchase_list_section', 'innerHTML', 'Purchase::showList', 'criteria=all-purchases&filterName=agent_id&filterValue=<?php echo $this->id 
		?>');
	</script>
<?php
			}
		}
		else				// agent ID is not exsiting, redirect to home page
		{
?>
		<script type="text/javascript">
		<!--
...
<?php
	...
	public static function showList()
	{
		...
		// MySQL bug: cannot run count and two left join
		...
		$sqlQuery = $sqlQuery . " ORDER BY " . $sortColumn . " " . $sortMethod;
...
?>			<td>
				...
				<a href="view_agent_details.php?id=<?php echo $agent['id'] ?>" title="<?php echo capitalizeWords( Filter::output( $agent['name'] ) ) ?>"><?php echo capitalizeWords( Filter::output( $agent['name'] ) ) ?></a>
				</span>
			</td>
...
<?php
	...
	public static function exportListToExcel( $username )
	{
		...
		// 1st query to get purchase count
		...
		// 2nd query to get agent details and order count
	...
	// show agent list for auto-suggest, ajax function
