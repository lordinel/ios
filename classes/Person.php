...
//----------------------------------------------------------------------------------------------------------------
// definition for person base class
//----------------------------------------------------------------------------------------------------------------
abstract class Person extends Layout
{
	...
	//------------------------------------------------------------------------------------------------------------
	// display input field set for basic info
	//------------------------------------------------------------------------------------------------------------
	protected static function showBasicInputFields( array $personInfo = null, $nameLabel = 'Name', $showContactPerson = false,
													$addressLabel = null, $addressIsRequired = true )
	{
		...
		<section>
			<div>
				...
				<input type="text" name="<?php echo $class ?>_name" id="<?php echo $class ?>_name" list="autosuggest_<?php echo $class ?>_name"
					   class="form_input_text" maxlength="100" autofocus="autofocus" required="required"<?php
							echo ($personInfo != null) ? ' value="' . capitalizeWords(Filter::reinput($personInfo['name'])) . '"' : '' ?> />
				<datalist id="autosuggest_<?php echo $class ?>_name"></datalist>
				<input type="hidden" name="<?php echo $class ?>_query_mode" id="<?php echo $class ?>_query_mode"
					   value="<?php echo ($personInfo != null) ? 'edit' : 'new' ?>" />
				<input type="hidden" name="<?php echo $class ?>_id" id="<?php echo $class ?>_id"
					   value="<?php echo ($personInfo != null) ? $personInfo['id'] : 'null' ?>" />
			</div>
			<?php
			if ($showContactPerson == true) {
				?>
				<div>
					<label for="contact_person">Contact Person:</label>
					<input type="text" name="contact_person" id="contact_person" class="form_input_text" maxlength="100"<?php
						echo ($personInfo != null) ? ' value="' . capitalizeWords(Filter::reinput($personInfo['contact_person'])) . '"' : '' ?> />
				</div>
				<?php
			}
			?>
		...
		if ($addressLabel != null) {
			...
			<section>
				<div>
					...
					<textarea name="address" id="address" rows="2"<?php
						echo ($addressIsRequired == true) ? 'required="required"' : '' ?>><?php
						echo ($personInfo != null) ? capitalizeWords(Filter::reinput($personInfo['address'])) : '' ?></textarea>
				</div>
			</section>
			<?php
		}
		...
		<section>
			<div>
				...
				<input type="tel" name="telephone" id="telephone" maxlength="100"<?php
					echo ($personInfo != null) ? ' value="' . Filter::reinput($personInfo['telephone']) . '"' : '' ?> />
			</div>
			<div>
				<label for="mobile">Mobile:</label>
				<input type="tel" name="mobile" id="mobile" maxlength="100"<?php
					echo ($personInfo != null) ? ' value="' . Filter::reinput($personInfo['mobile']) . '"' : '' ?> />
			</div>
			<div>
				<label for="fax">Fax:</label>
				<input type="tel" name="fax" id="fax" maxlength="100"<?php
					echo ($personInfo != null) ? ' value="' . Filter::reinput($personInfo['fax']) . '"' : '' ?> />
			</div>
			<div>
				<label for="email">E-mail:</label>
				<input type="email" name="email" id="email" maxlength="100"<?php
					echo ($personInfo != null) ? ' value="' . Filter::reinput($personInfo['email']) . '"' : '' ?> />
			</div>
		</section>
		<?php
	}
