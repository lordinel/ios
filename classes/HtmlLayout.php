...
class HtmlLayout
{
	...
	//------------------------------------------------------------------------------------------------------------
	// set page title, static function
	//------------------------------------------------------------------------------------------------------------
	...
	public function showMainMenu() {
		...
		echo '<div id="user_info"><span id="user_info_name">' . $this->user->getUserNameStatic() . '</span><br />' .
			 '<span id="user_info_action">[ <a class="nolinkdecor" href="account_settings.php">Account Settings</a> ] ' .
			 '[ <a class="nolinkdecor" href="index.php?action=logout">Logout</a> ]</span>' .
			 '</div></div></div>';
		...
		if (CLIENT == 'Express Dymans' &&
			$this->user->checkPermission(array('daily_sales_report',
											   'periodic_sales_report',
											   'projected_collections_report',
											   'inventory_report',
											   'revenue_and_expense_report',
											   'profit_calculator'), $database)) {
		...
		if (strtolower($this->pageName) != 'login') {
			...
			<div id="footer">
				<div id="footer_content">
					...
					if ($this->user->checkPermission('orders_and_customers', $database)) {
						echo ' | <a class="nolinkdecor" href="list_orders.php">Orders</a>' .
							 ' | <a class="nolinkdecor" href="list_customers.php">Customers</a>';
					}
					
					if ($this->user->checkPermission('purchases_and_suppliers', $database)) {
						echo ' | <a class="nolinkdecor" href="list_purchases.php">Purchases</a>' .
							 ' | <a class="nolinkdecor" href="list_suppliers.php">Suppliers</a>';
					}
