...
END;
		...
		echo "<form name=\"purchase_supplies\" method=\"post\" action=\"view_purchase_details.php\" autocomplete=\"off\" onsubmit=\"return order.validateInputForm(". ( $isItemEditable? 'true' : 'false' ) . ")\" onreset=\"return confirmReset('order.resetInputForm')\">\n";
		...
		// display supplier fieldset
		Supplier::showInputFieldSet( $supplierID );
		
		// display purchase fieldset
...
END;
	...
	public static function showInputFieldSet( $id = null, array $purchase = null, $isItemEditable = true )
	{
		...
		// display fieldset
		echo '<fieldset><legend>Purchase Info</legend>';
		echo '<section>';
		
		if ( $id != null )
		{
			echo '<div>';
			echo '<label for="purchase_number">Purchase No:</label>';
          	echo '<output name="purchase_number">' . $purchase['id'] . '</output>';
         	echo '</div>';
		}

?>        	<div>
				<label for="purchase_date" class="required_label">Purchase Date:</label><?php
				if ( $id == null )
	            {
?>				<input type="text" name="purchase_date" id="purchase_date" class="datepicker_no_future_date" size="30" maxlength="30" required="required" />
<?php			}
				else
				{
?>				<output name="purchase_date" id="purchase_date"><?php echo dateFormatOutput( $purchase['purchase_date'], "F j, Y, D" ) ?></output>
<?php			}
				
?>              <input type="hidden" name="purchase_query_mode" id="purchase_query_mode" value="<?php echo ( $id != null ) ? "edit" : "new" ?>" />
				<input type="hidden" name="purchase_id" id="purchase_id" value="<?php echo ( $id != null ) ? $purchase['id'] : "0" ?>" />
			</div>
            
            <div>
				<label for="invoice_type">Tracking No:</label>
                <select name="invoice_type" id="invoice_type" class="form_input_select">
                    <option value="SI"<?php echo ( $id != null ? ( $purchase['sales_invoice'] != null ? " selected=\"selected\"" : "" ) : "" ) ?>>Sales Invoice (SI)</option>
                    <option value="DR"<?php echo ( $id != null ? ( $purchase['delivery_receipt'] != null ? " selected=\"selected\"" : "" ) : "" ) ?>>Delivery Receipt (DR)</option>
                </select>
                <input type="hidden" name="invoice_type_orig" id="invoice_type_orig" value="<?php echo ( $id != null ) ? ( $purchase['sales_invoice'] != null ?  "SI" : "DR" ) : "null" ?>" />
            </div>
            
            <div>
            	<label for="tracking_number"></label>
				<input type="text" name="tracking_number" id="tracking_number"<?php echo ( $id != null ) ? " value=\"" . ( $purchase['sales_invoice'] != null ?  Filter::reinput( $purchase['sales_invoice'] ) : Filter::reinput( $purchase['delivery_receipt'] ) ) . "\"" : "" ?> />
                <input type="hidden" name="tracking_number_orig" id="tracking_number_orig" value="<?php echo ( $id != null ) ? ( $purchase['sales_invoice'] != null ?  Filter::reinput( $purchase['sales_invoice'] ) : Filter::reinput( $purchase['delivery_receipt'] ) ) : "null" ?>" />
			</div>
		</section>
         ...
         <section>
        	<div>
				...
				<input type="text" name="supplier_po_number" id="supplier_po_number" required="required"<?php echo ( $id != null ) ? " value=\"" . Filter::reinput( $purchase['purchase_number'] ) . "\"" : "" ?> />
			</div>
...
?>				<select name="transaction_type" id="transaction_type" class="form_input_select">
					<option value="delivery"<?php echo ( $id != null ? ( $purchase['transaction_type'] == "delivery" ? " selected=\"selected\"" : "" ) : "" ) ?>>Delivery</option>
					<option value="pick-up"<?php echo ( $id != null ? ( $purchase['transaction_type'] == "pick-up" ? " selected=\"selected\"" : "" ) : "" ) ?>>Pick-up</option>
				</select>
<?php			}
				else
				{
?>				<output><?php echo ucfirst( $order['transaction_type'] ) ?></output>
				<input type="hidden" name="transaction_type" id="transaction_type" value="<?php echo $purchase['transaction_type'] ?>" />
<?php			}
?>			</div>
...
<?php
				if ( $purchase['delivery_pickup_actual_date'] == null )
				{
?>				<input type="text" name="delivery_pickup_date" id="delivery_pickup_date" class="datepicker" size="30" maxlength="30" required="required"<?php echo ( $id != null ) ? " value=\"" . dateFormatOutput( Filter::reinput( $purchase['delivery_pickup_target_date'] ), "F j, Y, D" ) . "\"" : "" ?> />
<?php			}
				else
				{
					...
					echo '<input type="hidden" name="delivery_pickup_date" id="delivery_pickup_date" value="' . dateFormatOutput( Filter::reinput( $purchase['delivery_pickup_target_date'] ), "F j, Y, D" ) . '" />';
					echo '<input type="hidden" name="delivery_pickup_actual_time" id="delivery_pickup_actual_time" value="' . $purchase['delivery_pickup_actual_date'] . '" />';
				}
?>
			</div>
		</section>
...
<?php				
				...
				{
					$item = array();
					$resultSet = self::$database->query( "SELECT * FROM purchase_item INNER JOIN inventory ON purchase_item.inventory_id = inventory.id WHERE purchase_id = " . $purchase['id'] );
...
?>					<tr>
                    ...
                    <tr>
                        ...
                        <td><input type="text" name="total_sidr_amount" id="total_sidr_amount" class="number output_field" value="<?php echo ( $id != null ) ? numberFormat( Filter::reinput( $totalSIDRamount ), 'float', 3, '.', '', true ) : "0.000" ?>" disabled="disabled" /></td>
                        <td></td>
                        <td><input type="text" name="total_net_amount" id="total_net_amount" class="number output_field" value="<?php echo ( $id != null ) ? numberFormat( Filter::reinput( $totalNetAmount ), 'float', 3, '.', '', true ) : "0.000" ?>" disabled="disabled" /></td>
					</tr>
					...
					<tr class="tax_fields">
						...
						<td><input type="text" name="total_sales" id="total_sales" class="number order_item_total output_field" value="<?php echo ( $id != null ) ? numberFormat( Filter::reinput( $purchase['total_sales_amount'] ), 'float', 3, '.', '', true ) : "0.000" ?>" disabled="disabled" /></td>
                        <td></td>
					</tr>
                    <tr class="tax_fields">
						<td colspan="8"><label for="value_added_tax">Value-Added Tax: <?php echo CURRENCY ?></label></td>
						<td><input type="text" name="value_added_tax" id="value_added_tax" class="number order_item_total output_field" value="<?php echo ( $id != null ) ? numberFormat( Filter::reinput( $purchase['value_added_tax'] ), 'float', 3, '.', '', true ) : "0.000" ?>" disabled="disabled" /></td>
                    </tr>
                    <tr class="tax_fields">
						<td colspan="8"><label for="withholding_tax">Withholding Tax: <?php echo CURRENCY ?></label></td>
						<td><input type="text" name="withholding_tax" id="withholding_tax" class="number order_item_total" value="<?php echo ( $id != null ) ? numberFormat( Filter::reinput( $purchase['withholding_tax'] ), 'float', 3, '.', '', true ) : "0.000" ?>" disabled="disabled" /></td>
                    </tr>
					<tr class="tax_fields">
						<td colspan="9"><br /></td>
                    </tr>
					...
					<tr>
						...
						<td><input type="text" name="sidr_amount" id="sidr_amount" class="number order_item_total output_field" value="<?php echo ( $id != null ) ? numberFormat( Filter::reinput( $purchase['receipt_amount'] ), 'float', 3, '.', '', true ) : "0.000" ?>" disabled="disabled" /></td>
					</tr>
                    <tr>
						<td colspan="8"><label for="net_amount" class="important_label">OFC Net Amount: <?php echo CURRENCY ?></label></td>
						<td><input type="text" name="net_amount" id="net_amount" class="number order_item_total output_field" value="<?php echo ( $id != null ) ? numberFormat( Filter::reinput( $purchase['amount_payable'] ), 'float', 3, '.', '', true ) : "0.000" ?>" disabled="disabled" /></td>
					</tr>
				</tbody>
			</table>
		</section>
		...
		<!--
			order.setMaxItems( <?php echo self::MAX_PURCHASE_ITEMS ?> );
...
<?php
 ...
 Purchase Items and Payment details.</div></section>"; 
		}
	}
	...
	public function save( $supplierID )
	{
		...
		if ( $_POST['purchase_query_mode'] == "new" )			// new transaction
		{
			// save new order
			...
			//$purchaseNumber = str_pad( $this->id, 10, '0', STR_PAD_LEFT );
			$purchaseNumber = $this->id;
			if ( $this->salesInvoice != "NULL" ) {
				$invoiceNumber = ' (SI ' . $this->salesInvoice . ')';
			} elseif ( $this->deliveryReceipt != "NULL" ) {
				$invoiceNumber = ' (DR ' . $this->deliveryReceipt . ')';
			} else {
				$invoiceNumber = '';
			}
...
<?php
	...
	// view purchase list, ajax function
	public static function showList()
	{
		...
		if ( self::$database->getResultCount( $resultSet ) > 0 ) {
			...
			while ( $purchase = self::$database->getResultRow( $resultSet ) ) {
				...
				if ( $filterName != "supplier_id" ) {
					echo '<td>' .
						 ...
						 '<a href="view_supplier_details.php?id=' . $purchase['supplier_id'] . '" title="' . capitalizeWords( Filter::output( $purchase['supplier'] ) ) . '">' .
			...
			self::showPagination( $page, $itemsPerPage, $resultCount, 'purchase_list_section', 'Purchase::showList', $criteria, $sortColumn, $sortMethod, $filterName, $filterValue );
			...
			if ( $filterName != "supplier_id" && $filterName != "agent_id" ) {
				...
				echo '<div>' .
					 ...
					 '<span class="record_data">' . numberFormat( $purchase['total_rebate_receivable'] + $purchase['total_pdc_rebate_receivable'], "currency" ) . '</span>' .
					 '</div>';
				...
				if ( $purchase['delivery_pickup_actual_date'] != null ) {
					...
					if ( $deliveryPickupTargetDate < $deliveryPickupActualDate ) {
						...
						$activeSheet->getComment( 'J' . $rowPtr )->getText()
							->createTextRun( 'Target '.( $purchase['transaction_type'] == 'delivery' ? 'delivery' : 'pick-up' ) . ' date has been missed' )
							->getFont()->setSize( EXCEL_COMMENT_FONT_SIZE );
					}
				} else {
					$activeSheet->getComment( 'J' . $rowPtr )->getText()
						->createTextRun( 'Purchased items are not yet '.( $purchase['transaction_type'] == 'delivery' ? 'delivered by supplier' : 'picked-up from supplier' ) )
						->getFont()->setSize( EXCEL_COMMENT_FONT_SIZE );
				}
	...
	public function showDetailsTasks()
	{
		...
		{
			$sqlQuery = "SELECT clearing_actual_date FROM purchase_payment WHERE purchase_id = " . $this->id . " AND clearing_actual_date IS NULL AND amount >= 0";
			$resultSet = self::$database->query( $sqlQuery );
...
<?php
	...
	{
		// get main order info
		$sqlQuery = "SELECT purchase.*, IF(canceled_date IS NOT NULL,'canceled',IF(cleared_date IS NOT NULL,'cleared',IF(delivery_pickup_actual_date IS NULL,'pending','delivered'))) AS status, " .
...
?>	<fieldset><legend>Purchase Info</legend>
		...
		<section>
			...
			<div>
				...
				<span class="record_data"><a href="view_supplier_details.php?id=<?php echo $purchase['supplier_id'] ?>"><?php echo capitalizeWords( Filter::output( $purchase['supplier_name'] ) ) ?></a></span>
			</div>
            <div>
        ...
        <section>
			<div>
				...
				<span class="record_data"><?php echo ( $purchase['purchase_number'] != null ? Filter::reinput( $purchase['purchase_number'] ) : "(none)" ) ?></a></span>
			</div>
		</section>
                        ...
                        <td class="quantity_link"><?php
							if ( $purchase['status'] != "cleared" )
							{
                            	echo '<span class="item_delivery_link">' . 
									 '<a href="javascript:void(0)" onclick="showDialog(\'Mark Item as ' . ( $purchase['transaction_type'] == "delivery" ? "Received" : "Picked-up" ) . '\',\'Getting data...\',\'prompt\'), ajax(null,\'dialog_message\',\'innerHTML\',\'Purchase::showItemDeliveryDialog\',\'class=purchase&transactionID=' . $purchase['id'] . '&itemID=' . $item['id'] . '&index=' . $i . '\'), loadDatePicker()" >' .
...
<?php
					if ( $purchase['status'] != "cleared" )
					{
						?><span class="item_delivery_link"><a href="javascript:void(0)" onclick="showDialog('Mark All Items as <?php echo ( $purchase['transaction_type'] == "delivery" ? "Received" : "Picked-up" ) ?>','Getting data...','prompt'), ajax(null,'dialog_message','innerHTML','Purchase::showAllItemsDeliveryDialog','class=purchase&transactionID=<?php echo $purchase['id'] ?>&maxIndex=<?php echo ( $i - 1 ) ?>'), loadDatePicker()" ><span id="item_quantity_total"><?php echo $totalDeliveredItems ?></span></a></span><?php
					}
					else
					{
