...
class Supplier extends Person
{
	...
	{
		echo "<form name=\"" . ( $id == null ? "add" : "edit" ) . "_supplier\" method=\"post\" action=\"view_supplier_details.php\" autocomplete=\"off\" onreset=\"return confirmReset('resetSupplierForm')\">\n";
	...
	{
		if ( $_POST['supplier_query_mode'] == "new" )		// new supplier
		{
			...
			EventLog::addEntry( self::$database, 'info', 'supplier', 'insert', 'new',
								'<span class="event_log_main_record_inline"><a href="view_supplier_details.php?id=' . $this->id . '">' .
								capitalizeWords( htmlentities( $_POST['supplier_name'] ) ) .
								'</a></span> was <span class="event_log_action">added</span> to <a href="list_suppliers.php">Suppliers</a>' );
		}
		elseif ( $_POST['supplier_query_mode'] == "edit" )		// existing supplier, update records
		{
			...
			EventLog::addEntry( self::$database, 'info', 'supplier', 'update', 'modified',
								'Supplier <span class="event_log_main_record_inline"><a href="view_supplier_details.php?id=' . $this->id . '">' .
								capitalizeWords( htmlentities( $_POST['supplier_name'] ) ) .
	...
	// tasks for supplier details
	public function showDetailsTasks()
	{
?>		<div id="tasks">
			<ul>
				<li id="task_edit_customer"><a href="edit_supplier.php?id=<?php echo $this->id ?>"><img src="images/task_buttons/edit.png" />Edit Supplier</a></li>
...
<?php
	...
	public function view()
	{
		...
		if ( $supplier['id'] != null )		// supplier ID is found in database
		{
			...
			{
?>
	<fieldset><legend>Purchases List</legend>
	...
	<!--
		ajax( null, 'purchase_list_section', 'innerHTML', 'Purchase::showList', 'criteria=all-purchases&filterName=supplier_id&filterValue=<?php echo 
		$this->id ?>');
	// -->
	</script>
    	...
    	<section>
            <div class="report_data">
                <form name="report_date_form">
                    ...
                    <input type="text" name="startdate" id="startdate" class="datepicker_no_future_date" size="30" maxlength="30" required="required" value="<?php echo $startDateStr ?>" />
                    <label for="enddate">End Date:</label>
                    <input type="text" name="enddate" id="enddate" class="datepicker_no_future_date" size="30" maxlength="30" required="required" value="<?php echo $endDateStr ?>" />
                    <input type="button" name="submit_form" value="Go" onclick="javascript:ajax( null, 'supplier_statistics_section', 'innerHTML', 'Supplier::showHistoryAndStatistics', 'supplierID=<?php echo $this->id ?>&startDate=' + $('#startdate').val() + '&endDate=' + $('#enddate').val() )" />
                </form>
            </div>
        </section>
    ...
    <script type="text/javascript">
	<!--		
		ajax( null, 'supplier_statistics_section', 'innerHTML', 'Supplier::showHistoryAndStatistics', 'supplierID=<?php echo $this->id ?>&startDate=<?php echo $startDateStr ?>&endDate=<?php echo $endDateStr ?>' );
	// -->
	</script>
<?php
			}
		}
		else				// customer ID is not exsiting, redirect to home page
		{
?>
		<script type="text/javascript">
		<!--
...
<?php
	...
	// view supplier list, ajax function
		...
		switch ( $criteria ) {
			...
			default:
				...
				switch ( $criteria ) {
					...
					case "without-rebate" :
						$condition = $condition . "(v_supplier_payment_summary.rebate_receivable = 0 AND v_supplier_payment_summary.pdc_rebate_receivable = 0) ";
						break;
				}
		...
		if ( self::$database->getResultCount( $resultSet ) > 0 ) {
			...
			while ( $supplier = self::$database->getResultRow( $resultSet ) )
			{
				...
				if ( $supplier['pdc_rebate_receivable'] > 0 ) {
					echo '<img src="images/rebate.png" class="status_icon" title="Rebate to clear: ' . numberFormat( $supplier['pdc_rebate_receivable'], "currency", 3, '.', ',', true ) . '" />';
				}
				echo '</td>';
			...
			self::showPagination( $page, $itemsPerPage, $resultCount, 'supplier_list_section', 'Supplier::showList', $criteria, $sortColumn, $sortMethod, $filterName, $filterValue );
		}
		else
		{
	...
	public static function exportListToExcel( $username, $paramArray )
	{
		...
		$sqlQuery = "SELECT supplier.*, " .
					...
					"LEFT JOIN v_supplier_payment_summary ON supplier.id = v_supplier_payment_summary.supplier_id " .
					$condition .
					"GROUP BY supplier.id ORDER BY supplier.name ASC";
	...
	// show supplier list for auto-suggest, ajax function
	...
	public static function autoFill()
	{
		...
		if ( $supplier != null ) {
			$supplier['contact_person'] = html_entity_decode( capitalizeWords( Filter::output( $supplier['contact_person'] ) ) );
			$supplier['address'] = html_entity_decode( capitalizeWords( Filter::output( $supplier['address'] ) ) );
