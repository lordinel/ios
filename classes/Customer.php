...
class Customer extends Person
{
	...
	public static function showInputForm( $id = null ) {
		echo '<form name="' . ($id == null ? 'add' : 'edit') .
			 '_customer" method="post" action="view_customer_details.php" autocomplete="off" onreset="return confirmReset(\'resetCustomerForm\')">';
	...
	//------------------------------------------------------------------------------------------------------------
	// display customer form field set
	//------------------------------------------------------------------------------------------------------------
	public static function showInputFieldset( $id = null, array $order = null ) {
		...
		<section>
			<div>
				...
				<input type="text" name="credit_limit" id="credit_limit" class="number" maxlength="100" required="required" value="<?php
					echo ($customerInfo != null) ? numberFormat(Filter::reinput($customerInfo['credit_limit']), 'float', 3, '.', '', true) : "0.000" ?>" />
    	        ...
    	        <input type="text" id="remaining_credits" class="number output_field" disabled="disabled" value="<?php
					echo($customerInfo != null ? numberFormat($customerInfo['credit_remaining'], 'float', 3, '.', '', true) : "0.000") ?>" />
                </span>
			</div>
			<div>
				<label for="credit_terms">Credit Terms:</label>
				<select name="credit_terms" id="credit_terms" class="form_input_select">
					<option value="30 days"<?php echo($customerInfo != null ?
						($customerInfo['credit_terms'] == '30 days' ? ' selected="selected"' : '') : ' selected="selected"') ?>>30 days</option>
					<option value="60 days"<?php echo($customerInfo != null ?
						($customerInfo['credit_terms'] == '60 days' ? ' selected="selected"' : '') : '') ?>>60 days</option>
					<option value="90 days"<?php echo($customerInfo != null ?
						($customerInfo['credit_terms'] == '90 days' ? ' selected="selected"' : '') : '') ?>>90 days</option>
					<option value="120 days"<?php echo($customerInfo != null ?
						($customerInfo['credit_terms'] == '120 days' ? ' selected="selected"' : '') : '') ?>>120 days</option>
					<option value="150 days"<?php echo($customerInfo != null ?
						($customerInfo['credit_terms'] == '150 days' ? ' selected="selected"' : '') : '') ?>>150 days</option>
				</select>
			</div>
		</section>
		<section>
			...
			<div>
				...
				<span class="form_hint">
					...
					<span class="now">Note:</span> You can only view the customer and the corresponding orders if 
					you and the customer belong to the same branch.</span>
				</span>
			</div>
		</section>
		</fieldset><?php
	...
	//------------------------------------------------------------------------------------------------------------
	// auto-suggest customer name; ajax function
	//------------------------------------------------------------------------------------------------------------
	...
	//------------------------------------------------------------------------------------------------------------
	// auto-fill customer form based on auto-suggest selection; ajax function
	//------------------------------------------------------------------------------------------------------------
	public static function autoFill() {
		...
		if ($customer == null) {
			...
			$customer['contact_person']   = html_entity_decode(capitalizeWords(Filter::reinput($customer['contact_person'])));
			$customer['address']          = html_entity_decode(capitalizeWords(Filter::reinput($customer['address'])));
	...
	public function save() {
		if ($_POST['customer_query_mode'] == 'new') {
			// new customer
			...
			EventLog::addEntry(self::$database, 'info', 'customer', 'insert', 'new',
							   ...
							   '<a href="view_customer_details.php?id=' . $this->id . '">' . capitalizeWords(htmlentities($_POST['customer_name'])) . '</a>' .
							   '</span> was <span class="event_log_action">added</span> to <a href="list_customers.php">Customers</a>');
			
		} elseif ($_POST['customer_query_mode'] == 'edit') {
			...
			EventLog::addEntry(self::$database, 'info', 'customer', 'update', 'modified',
							   'Customer <span class="event_log_main_record_inline">' .
							   '<a href="view_customer_details.php?id=' . $this->id . '">' . capitalizeWords(htmlentities($_POST['customer_name'])) . '</a>' .
	...
	//------------------------------------------------------------------------------------------------------------
	// display tasks for customer list
	//------------------------------------------------------------------------------------------------------------
	...
	//------------------------------------------------------------------------------------------------------------
	// display list of customers; ajax function
	//------------------------------------------------------------------------------------------------------------
	public static function showList() {
		...
		$sqlQuery = "SELECT customer.*, " .
					...
					"LEFT JOIN v_customer_payment_summary ON customer.id = v_customer_payment_summary.customer_id " .
					"$condition GROUP BY customer.id " .
		...
		while ($customer = self::$database->getResultRow($resultSet)) {
			...
			if ($customer['pdc_rebate_payable'] > 0) {
				echo '<img src="images/rebate.png" class="status_icon" title="Rebate to clear: ' .
					 numberFormat($customer['pdc_rebate_payable'], 'currency', 3, '.', ',', true) . '" />';
			}
			echo '</td>';
		...
		self::showPagination($page, $itemsPerPage, $resultCount, 'customer_list_section', 'Customer::showList',
							 $criteria, $sortColumn, $sortMethod, $filterName, $filterValue);
	}
	
	
	//------------------------------------------------------------------------------------------------------------
	// save customer list to excel file; ajax function
	//------------------------------------------------------------------------------------------------------------
	public static function exportListToExcel( $username, $paramArray ) {
		...
		$sqlQuery = "SELECT customer.*, " .
					...
					"LEFT JOIN v_active_orders ON customer.id = v_active_orders.customer_id " .
					"LEFT JOIN v_customer_payment_summary ON customer.id = v_customer_payment_summary.customer_id " .
					"$condition GROUP BY customer.id ORDER BY customer.name ASC";
		$resultSet = self::$database->query($sqlQuery);
	...
	//------------------------------------------------------------------------------------------------------------
	// display tasks for customer details
	//------------------------------------------------------------------------------------------------------------
	public function showDetailsTasks() {
		?>
		<div id="tasks">
			<ul>
				<li id="task_edit_customer"><a href="edit_customer.php?id=<?php echo $this->id ?>">
					<img src="images/task_buttons/edit.png" />Edit Customer</a></li>
	...
	public function view() {
		...
		if ($customer['id'] == null) {
			// customer ID is not found in database
			redirectToHomePage();
		}
		...
		// get orders made by this customer
		...
		if ((int) $order['count'] > 0) {
			...
			<!--
				ajax(null, 'order_list_section', 'innerHTML', 'Order::showList',
					 'criteria=all-orders&filterName=customer_id&filterValue=<?php echo $this->id ?>');
			// -->
			</script>
				...
				<section>
					<div class="report_data">
						<form name="report_date_form">
							...
							<input type="text" name="startdate" id="startdate" class="datepicker" size="30" maxlength="30" required="required"
								   value="<?php echo $startDateStr ?>"/>
							<label for="enddate">End Date:</label>
							<input type="text" name="enddate" id="enddate" class="datepicker" size="30" maxlength="30" required="required"
								   value="<?php echo $endDateStr ?>"/>
							<input type="button" name="submit_form" value="Go"
								   onclick="javascript:ajax(null, 'customer_statistics_section', 'innerHTML',
															'Customer::showHistoryAndStatistics',
															'customerID=<?php echo $this->id ?>&startDate=' +
			...
			<script type="text/javascript">
			<!--
				ajax(null, 'customer_statistics_section', 'innerHTML', 'Customer::showHistoryAndStatistics',
					 'customerID=<?php echo $this->id ?>&startDate=<?php echo $startDateStr ?>&endDate=<?php echo $endDateStr ?>');
			// -->
			</script><?php
		}
	}
	...
	//------------------------------------------------------------------------------------------------------------
	// display order history and statistics for customer; ajax function
	//------------------------------------------------------------------------------------------------------------
