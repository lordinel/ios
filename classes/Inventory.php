...
class Inventory extends Layout
{
	...
	public static function loadModelListSuggestion() {
		...
		if ($_POST['brandID'] != 0) {
			...
			echo '<form name="model_suggestion_form" id="model_suggestion_form" ' .
				 ...
				 '<span class="record_data"><input type="text" name="model_suggestion" id="model_suggestion" required="required" ' .
				 	'autofocus="autofocus" size="45" maxlength="100" list="autosuggest_model" value="' .
				 	$_POST['defaultTextValue'] . '" /></span>' .
				 '</div><br /><br /><br />' .
	...
	public static function loadSellingPriceAndStock() {
		...
		if ($_POST['inventoryID'] != 0) {
			...
			$resultSet 		= self::$database->query("SELECT ROUND(selling_price,3) AS selling_price, stock_count AS available_stock, " .
												"reserved_stock FROM inventory WHERE id = " . $_POST['inventoryID']);
	...
	private static function showAddEditBrandDialog( $brandID = null, $brandName = null ) {
		$dialogMessage = '<form name="add_edit_brand" method="post" action="javascript:' .
						 ($brandID == null ? "addBrandCommit()" : "editBrandCommit()") .
						 '" autocomplete="off">' .
	...
	public static function showBrandList() {
		...
		self::showPagination($page, $itemsPerPage, $resultCount, 'brand_list_section', 'Inventory::showBrandList', null, $sortColumn, $sortMethod, $filterName,
							 $filterValue);
	}
	...
	public static function showModelListTasks( $brandID = null, $brandName = null ) {
		...
		?><div id="tasks">
		<ul>
			<li id="task_add_brand">
				<a href="javascript:void(0)" onclick="showDialog('Add Model for <?php echo capitalizeWords(Filter::reinputToJS($brandName)) ?>',
										   				   		 '<?php self::showAddEditModelDialog($brandID) ?>','prompt'); $('#new_model_name').focus()">
				<img src="images/task_buttons/add.png" />Add Model...</a></li>
			<li id="task_edit_brand">
				<a href="javascript:void(0)" onclick="showDialog('Edit Brand','<?php self::showAddEditBrandDialog($brandID, $brandName) ?>',
																 'prompt'); $('#new_brand_name').select()">
				<img src="images/task_buttons/edit.png" />Change Brand Name...</a></li>
			<li id="task_delete_brand"><a href="javascript:void(0)" onclick="showDialog('Delete Brand','<?php
				...
				$dialogMessage = 'Delete <b>' . capitalizeWords(Filter::reinputToJS($brandName)) . '</b> and all its associated models?<br /><br />' .
								 ...
								 '<div id="dialog_buttons">' .
								 '<input type="button" value="Yes" onclick="deleteBrandCommit(\\\'' . $brandID . '\\\', \\\'' .
								 addslashes(capitalizeWords(Filter::reinputToJS($brandName))) . '\\\')" />' .
								 '<input type="button" value="No" onclick="hideDialog()" />' .
								 '</div>';
				$dialogMessage = htmlentities($dialogMessage);
				echo $dialogMessage;
		...
		<span style="visibility: hidden" id="formatted_new_brand_name"><?php echo capitalizeWords(Filter::output($brandName)); ?></span><?php
	}
		...
		if ($parentID == null) {
			...
			$dialogMessage = '<form name="add_edit_model" method="post" action="javascript:' .
							 ...
							 ($modelID != null ? '<input type="button" value="Delete Model..." ' .
												 'onclick="deleteModelConfirm(\'' . $brandID . '\',\'' . $modelID . '\',\'' . addslashes($modelName) .
	...
	public static function showModelList() {
		...
		while ($model = self::$database->getResultRow($resultSet)) {
			...
			?><td><span class="long_text_clip">
				<a href="javascript:void(0)" onclick="showDialog('Edit Model',
					'<?php self::showAddEditModelDialog($brandID, $model['id'],
													    ...
													    $model['stock_count'],
													    $model['parent_id']) ?>','prompt')"
			   	title="<?php echo capitalizeWords(Filter::output($model['model'])) ?>">
	...
	public static function getBrandName( $brandID ) {
		...
		$sqlQuery = "SELECT name FROM inventory_brand WHERE inventory_brand.id = " . $brandID;;
		$resultSet = self::$database->query($sqlQuery);
	...
	public static function showDeleteModelDialog() {
		...
		if ($resultCount > 0) {
			echo '<hr /><br />' .
				 ...
				 '<span class="record_data"><input type="text" name="duplicate_inventory" id="duplicate_inventory" ' .
				 'size="45" maxlength="100" list="autosuggest_model" disabled="disabled" /></span>' .
				 '</div><br />';
		...
		echo '<div id="dialog_buttons"><br />' .
			 '<input type="button" value="Yes" onclick="deleteModelCommit(' . $resultCount . ',\'' . $_POST['brandID'] . '\',\'' .
			 															  $_POST['modelID'] . '\',\'' . addslashes(Filter::output($_POST['modelName'])) .
			 															  '\')" />' .
			 '<input type="button" value="No" onclick="hideDialog()" />' .
			 '</div></form>';
	}
	...
	public static function addBrand() {
		...
		if (self::$database->getResultCount($resultSet) > 0) {
			echo '<b>Error:</b> Cannot add <b>' . capitalizeWords($_POST['newBrandName']) . '</b><br /><br />' .
				 'Ensure that the new brand name is not the same as the existing brands.<br /><br />' .
		...
		if (self::$database->query($sqlQuery)) {
			...
			echo '<b>' . capitalizeWords(htmlentities($_POST['newBrandName'])) .
				 '</b> successfully added to Brand list!<br /><br />Redirecting to model list...<br />';
	...
	public static function editBrand() {
		...
		$resultSet = self::$database->query($sqlQuery);
		if (self::$database->getResultCount($resultSet) > 0) {
			echo '<b>Error:</b> Cannot set <b>' . capitalizeWords(htmlentities($_POST['newBrandName'])) . '</b> as new brand name for <b>' .
				 htmlentities($_POST['oldBrandName']) . '</b><br /><br />' .
				 'Ensure that the new brand name is not the same as the existing brands<br /><br />' .
				 'Please verify and try again.';
			return;
		}
		...
		if (self::$database->query($sqlQuery)) {
			...
			echo capitalizeWords(htmlentities($_POST['oldBrandName'])) .
				 ' brand was renamed to <b>' .
				 capitalizeWords(htmlentities($_POST['newBrandName'])) .
				 '</b> successfully!<br /><br /><br />';
		}
	}
	...
	public static function deleteBrand() {
		...
		if (self::$database->query($sqlQuery)) {
			...
			echo '<b>' . capitalizeWords(htmlentities($_POST['brandName'])) .
				 '</b> has been deleted from brand list!<br /><br />Redirecting to brand list...<br />';
		} else {
			echo '<b>Error:</b> Cannot delete ' . capitalizeWords(htmlentities($_POST['brandName'])) . ' to brand list<br /><br />' .
				 'It is possible that this brand is currently in use in some orders.<br />' .
				 'Please verify and try again.';
		}
	}
	...
	public static function addModel() {
		...
		if (self::$database->getResultCount($resultSet) > 0) {
			echo '<b>Error:</b> Cannot add <b>' . capitalizeWords(htmlentities($_POST['newModelName'])) . '</b><br /><br />' .
				 'Ensure that the new model name is not the same as any of the existing model names within the same brand.<br /><br />' .
				 'Please verify and try again.';
			return;
		}
	...
	public static function exportListToExcel( $username, $paramArray = null ) {
		...
		if (self::$database->getResultCount($resultSet) > 0) {
			...
			for ($i = 0; $i <= $inventoryBrandCount; $i++) {
				...
				$activeSheet->getStyle('A' . $FIELD_HEADER_ROW . ':' . $MAX_COLUMN . $FIELD_HEADER_ROW)->getFill()
				...
				// post formatting
				$activeSheet->getStyle('A' . $ROW_START_OF_DATA . ':B' . $rowPtr)->getAlignment()
							->setWrapText(true);                        // wrap Model and Description
				$activeSheet->getStyle('C' . $ROW_START_OF_DATA . ':D' . $rowPtr)->getNumberFormat()
							->setFormatCode(EXCEL_CURRENCY_FORMAT);    // format Purchase Price and Selling Price
				$activeSheet->getStyle('E' . $ROW_START_OF_DATA . ':E' . $rowPtr)->getNumberFormat()
							->setFormatCode(EXCEL_INT_FORMAT);        // format Available Stocks
				$activeSheet->getStyle('F' . $ROW_START_OF_DATA . ':H' . $rowPtr)->getNumberFormat()->setFormatCode(EXCEL_CURRENCY_FORMAT);    // format amounts
				$activeSheet->getStyle('F' . $ROW_START_OF_DATA . ':H' . $rowPtr)->getFont()
							->setBold(true);                                // set amounts to bold
				...
				$activeSheet->getStyle('J' . $ROW_START_OF_DATA . ':T' . $rowPtr)->getAlignment()
							->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_LEFT);    // Remarks
		...
		if ($brandID == null) {
			...
			$activeSheet->getStyle('B6:C' . $rowPtr)->getNumberFormat()
						->setFormatCode(EXCEL_INT_FORMAT);        											// format No. of Models and Total Available Stocks
	...
	private static function showModelButtonSet( $brandID, $brandName, $page = null, $itemsPerPage = null,
												$resultCount = null, $sortColumn = null, $sortMethod = null ) {
		?><div id="dialog_buttons" class="inventory_buttons">
			<input type="button" value="Add Model"
				   onclick="showDialog('Add Model for <?php echo Filter::reinputToJS($brandName) ?>',
					   				   '<?php self::showAddEditModelDialog($brandID) ?>','prompt')" />
			<input type="button" value="Edit Brand"
				   onclick="showDialog('Edit Brand','<?php self::showAddEditBrandDialog($brandID, Filter::reinputToJS($brandName)) ?>','prompt')" />
			<input type="button" value="Delete Brand" onclick="showDialog('Delete Brand','<?php
				...
				$dialogMessage = 'Delete <b>' . Filter::reinputToJS($brandName) . '</b> and all its associated models?<br /><br />' .
								 'Note: this will succeed only if no order is having this brand.<br /><br /><br />' .
								 '<div id="dialog_buttons">' .
								 '<input type="button" value="Yes" onclick="deleteBrandCommit(\\\'' . $brandID . '\\\', \\\'' . $brandName . '\\\')" />' .
								 '<input type="button" value="No" onclick="hideDialog()" /></div>';
				$dialogMessage = htmlentities($dialogMessage);
				echo $dialogMessage;
