...
$stableBuild = array('version' => '4.14',
					 'label'   => '4.14');

$builds = array('4.13'   => '4.13',
				'4.12'   => '4.12',
				'4.11'   => '4.11',
				'4.10'   => '4.10',
				'4.9'    => '4.9',
...
<fieldset>
	<section>
		<div id="about_content">
			...
			<ul>
				<?php
				$resultSet = $database->query("SELECT * FROM bugs WHERE blocking = '" . $stableBuild['version'] .
											  "' AND ( status != 'dropped' OR status IS NULL ) ORDER BY number DESC");
			...
			<ul>
				<?php
				$buildVersions = array_keys($builds);
				...
				foreach ($buildVersions as $version) {
					...
					<li><?php echo $builds[$version] ?>
						<ul>
							<?php
							$resultSet = $database->query("SELECT * FROM bugs WHERE blocking = '" . $version .
														  "' AND status = 'resolved' ORDER BY number DESC");
