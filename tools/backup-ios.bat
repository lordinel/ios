...
rem get data only
C:\wamp\bin\mysql\mysql5.6.17\bin\mysqldump -u root --databases ios --compact --complete-insert --extended-insert --no-create-db --no-create-info --order-by-primary --skip-comments --log-error="%backuppath%ios_dump_error.log" >> %backuppath%ios_%timestamp%.sql
